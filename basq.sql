...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ProductLanguages]') AND [c].[name] = N'ProductUrl');
IF @var3 IS NOT NULL EXEC(N'ALTER TABLE [ProductLanguages] DROP CONSTRAINT [' + @var3 + '];');
ALTER TABLE [ProductLanguages] DROP COLUMN [ProductUrl];
GO
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[CategoryLanguages]') AND [c].[name] = N'CategoryUrl');
IF @var4 IS NOT NULL EXEC(N'ALTER TABLE [CategoryLanguages] DROP CONSTRAINT [' + @var4 + '];');
ALTER TABLE [CategoryLanguages] DROP COLUMN [CategoryUrl];
GO

UPDATE [Carts] SET [Created_At] = '2020-11-16T11:15:07.8583967+07:00'
WHERE [Id] = 2;
SELECT @@ROWCOUNT;

GO

UPDATE [Roles] SET [ConcurrencyStamp] = N'c1c2e6be-9cb3-4564-bbd9-48dd7a65ef00'
WHERE [Id] = '8d04dce2-969a-435d-bba4-df3f325983dc';
SELECT @@ROWCOUNT;

GO

UPDATE [Roles] SET [ConcurrencyStamp] = N'5805df75-7899-4437-b17b-e60f61718716'
WHERE [Id] = '8d04dce2-969a-435d-bba4-df3f325983dd';
SELECT @@ROWCOUNT;

GO

UPDATE [Users] SET [ConcurrencyStamp] = N'e654f6f5-93af-4afe-b3ee-a91fba22393b', [PasswordHash] = N'AQAAAAEAACcQAAAAEB4pQJbfDBCVOnHVc1SAyJAMQZb3Oqtg6Phg3eEw+X6bzFJbohwDxTDZO9j+r3ZWaQ=='
WHERE [Id] = '69bd714f-9576-45ba-b5b7-f00649be00de';
SELECT @@ROWCOUNT;
...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20201116041508_fixDb', N'5.0.0');
GO
